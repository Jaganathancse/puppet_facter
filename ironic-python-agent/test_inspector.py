...
class TestCollectNumaTopologyInfo(test_base.BaseTestCase):
    ...
    @mock.patch.object(inspector, 'get_numa_nodes_nics_info', autospec=True)
    @mock.patch.object(inspector, 'get_numa_nodes_cores_info', autospec=True)
    @mock.patch.object(inspector, 'get_numa_nodes_memory_info', autospec=True)
    @mock.patch.object(os.path, 'isdir', autospec=True)
    def test_collect_success(self, mock_isdir, mock_memory_info, mock_cores_info, mock_nics_info, mock_listdir):
        ...
        mock_cores_info.return_value = [{'cpu':0, 'numa_node':0, 'thread_siblings':[0,1,2,3]},
                                        {'cpu':1, 'numa_node':1, 'thread_siblings':[4,5,6]}]
        mock_nics_info.return_value = [{'name':'enp01', 'numa_node':0},
                                       {'name':'ens01', 'numa_node':1}]
        expected_numa_topology = {
                                  "ram": [{'numa_node':0, 'size_kb':1560000 },
                                         {'numa_node':1, 'size_kb':1200000 }],
                                  "cpus": [{'cpu':0, 'numa_node':0, 'thread_siblings':[0,1,2,3]},
                                           {'cpu':1, 'numa_node':1, 'thread_siblings':[4,5,6]}],
                                  "nics": [{'name':'enp01', 'numa_node':0},
                                           {'name':'ens01', 'numa_node':1}]
                                 }
        ...
        with mock.patch('six.moves.builtins.open', mock_open):
            mock_read = mock_open.return_value.read
            inspector.collect_numa_topology_info(self.data, self.failures)
        self.assertEqual(expected_numa_topology, self.data["numa_topology"])

    def test_collect_no_numa_dirs(self, mock_listdir):
      mock_listdir.side_effect = OSError()
      inspector.collect_numa_topology_info(self.data, self.failures)
    ...
    @mock.patch.object(inspector, 'get_numa_nodes_memory_info', autospec=True)
    @mock.patch.object(os.path, 'isdir', autospec=True)
    def test_collect_no_nics_dirs(self, mock_isdir, mock_memory_info, mock_cores_info, mock_nics_info, mock_listdir):
        numa_node_dirs = ['node0', 'node1']
        mock_listdir.return_value = numa_node_dirs
        mock_isdir.return_value = True
        mock_memory_info.return_value = [{'numa_node':0, 'size_kb':1560000 },
        ...
        mock_cores_info.return_value = [{'cpu':0, 'numa_node':0, 'thread_siblings':[0,1,2,3]},
                                        {'cpu':1, 'numa_node':1, 'thread_siblings':[4,5,6]}]
        mock_nics_info.side_effect = OSError()
    ...
    @mock.patch.object(os.path, 'isdir', autospec=True)
    def test_get_numa_nodes_memory_info(self, mock_isdir, mock_listdir):
        numa_node_dirs = ['node0', 'node1']
        mock_listdir.return_value = numa_node_dirs
        mock_isdir.return_value = True
        reads = ['Node 0 MemTotal:       1560000 kB', 'Node 1 MemTotal:       1200000 kB']
        expected_meminfo = [{'numa_node':0, 'size_kb':1560000 },
                            {'numa_node':1, 'size_kb':1200000 }]
        mock_open = mock.mock_open()
        with mock.patch('six.moves.builtins.open', mock_open):
            mock_meminfo_file = mock_open.return_value.read
            mock_meminfo_file.side_effect = reads
            ram = inspector.get_numa_nodes_memory_info('/sys/devices/system/node/', numa_node_dirs)
        self.assertListEqual(expected_meminfo, ram)

    @mock.patch.object(os.path, 'isdir', autospec=True)
    def test_bad_numa_nodes_memory_info(self, mock_isdir, mock_listdir):
        numa_node_dirs = ['node0', 'node1']
        mock_listdir.return_value = numa_node_dirs
        mock_isdir.return_value = True
