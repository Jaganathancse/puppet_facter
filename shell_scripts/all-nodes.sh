...
function ping_controller_ips() {
  local REMOTE_IPS=$1
  interfaces=$(ovs-vsctl list bridge | egrep "name" | awk '{print $3}')
  #echo "$interfaces"
  for REMOTE_IP in $(echo $REMOTE_IPS | sed -e "s| |\n|g" | sort -u); do
    ...
    for LOCAL_NETWORK in $networks; do
      ...
      if [[ $in_network == "True" ]]; then
        ovs_netdev_network="False"
        for INTERFACE in $interfaces; do
          interface_name=${INTERFACE:1:length-1}
          if [[ $(ovs-vsctl get bridge "${interface_name}" datapath_type) == "netdev" ]]; then          
            INTERFACE_IPS=$(ifconfig "${interface_name}" | grep -i "inet" | awk '{print $2}')
            #echo "$INTERFACE_IPS"
            for interface_ip in $INTERFACE_IPS; do
              interface_in_network=$(python -c "import ipaddr; net=ipaddr.IPNetwork('$LOCAL_NETWORK'); interface_addr=ipaddr.IPAddress('$interface_ip'); print(interface_addr in net)")
              #echo "interface: $interface_in_network"
              if [[ $interface_in_network == "True" ]]; then
                ovs_netdev_network="True"
                echo "${REMOTE_IP} is in DPDK Network."
                exit 1
              fi
            done
          fi
        done
        if [[ $ovs_netdev_network == "False" ]]; then
...
ping_controller_ips "192.168.100.137"
